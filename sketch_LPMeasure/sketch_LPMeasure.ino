 ...
 */
 ...
 unsigned long LiquidProduction=0;//产液量，即累计流量。必须申明为全局变量，最大计量到43万方
 unsigned long T1=0;
 unsigned long T2=0;
 float LPS=0;//瞬时流量
  /*
 * 函数申明区域，实践证明arduino不要函数申明
 */ 
...
float LPMeasure()
{
     ...
     */
      if(weight1>=weightFull&&weight2<=weightEmpty)
      {
          ...
          LiquidProduction=LiquidProduction+PerVolume;//每次计量液量   
      }
      if(weight2>=weightFull&&weight1<=weightEmpty)
      {
       ...
       LPS=PerVolume*0.001*3600*1000/(T1-T2);//瞬时产液量，单位：方/小时
       return LPS;
}
  /*
 * 进行倒换罐进行储液程序
